...
def populate():
    ...
    try:
        dm_rules = load_json("dm_rules.json")
        world_rules = load_json("world_rules.json")
        fire_techniques = load_json("fire_techniques.json")
        narrative_events = load_json("narrative_events.json")
        liang_data = load_json("liang_wuzhao.json")

        populate_dm_guidelines(db_session, dm_rules)
        populate_world_lore(db_session, world_rules)
        populate_techniques(db_session, fire_techniques)
        populate_campaign_events(db_session, narrative_events)

        populate_character_liang_wuzhao(db_session, liang_data, fire_techniques)
